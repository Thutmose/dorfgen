...
public class MapGenUGRegions extends MapGenCaves
{
    ...
    {
        this.makeCave(seed, chunkx, chunkz, blocks, x, y, z, 1.0F + this.rand.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1,
                0.5D);
    }

    protected void makeCave(long seed, int chunkx, int chunkz, Block[] blocks, double x, double y, double z,
            float p_151541_12_, float p_151541_13_, float p_151541_14_, int p_151541_15_, int p_151541_16_,
            double p_151541_17_)
    {
        ...
        //
        // for (boolean flag = random.nextInt(6) == 0; p_151541_15_ <
        // p_151541_16_; ++p_151541_15_)
        // {
        // double d6 = 1.5D + (double)(MathHelper.sin((float)p_151541_15_ *
        // (float)Math.PI / (float)p_151541_16_) * p_151541_12_ * 1.0F);
        ...
        // p_151541_16_ > 0)
        // {
        // this.makeCave(random.nextLong(), chunkx, chunkz, blocks, x, y, z,
        // random.nextFloat() * 0.5F + 0.5F, p_151541_13_ - ((float)Math.PI /
        // 2F), p_151541_14_ / 3.0F, p_151541_15_, p_151541_16_, 1.0D);
        // this.makeCave(random.nextLong(), chunkx, chunkz, blocks, x, y, z,
        // random.nextFloat() * 0.5F + 0.5F, p_151541_13_ + ((float)Math.PI /
        // 2F), p_151541_14_ / 3.0F, p_151541_15_, p_151541_16_, 1.0D);
        // return;
        ...
        // }
        //
        // if (x >= d4 - 16.0D - d6 * 2.0D && z >= d5 - 16.0D - d6 * 2.0D && x
        // <= d4 + 16.0D + d6 * 2.0D && z <= d5 + 16.0D + d6 * 2.0D)
        // {
        ...
        // }
        //
        // if (i3 != j4 - 1 && k2 != i4 && k2 != l1 - 1 && l2 != k4 && l2 != j2
        // - 1)
        // {
        // i3 = j4;
        // }
    ...
    protected void func_151538_a(World world, int x, int z, int chunkX, int chunkZ, Block[] blocks)
    {
        ...
        for (int j1 = 0; j1 < i1; ++j1)
        {
            ...
            double d1 = -1;// (double)this.rand.nextInt(this.rand.nextInt(120) +
                           // 8);
            ...
            if (xAbs >= 0 && xAbs < WorldGenerator.instance.dorfs.elevationMap.length && zAbs >= 0
                    && zAbs < WorldGenerator.instance.dorfs.elevationMap[0].length)
            {
                h = heightInterpolator.interpolateHeight(WorldGenerator.scale, xAbs, zAbs,
                        WorldGenerator.instance.dorfs.elevationMap);
            }
            else
            {
                h = 64;// world.getGroundAboveSeaLevel(new BlockPos(d0,
                       // d2));//TODO
            }
            h /= 2;
    ...
    // //Determine if the block at the specified location is the top block for
    // the biome, we take into account
    ...
    // if(xAbs >=0 && xAbs <WorldGenerator.instance.dorfs.elevationMap.length &&
    // zAbs >=0 && zAbs <WorldGenerator.instance.dorfs.elevationMap[0].length)
    // {
    // int h = heightInterpolator.interpolateHeight(WorldGenerator.scale, xAbs,
    // zAbs, WorldGenerator.instance.dorfs.elevationMap);
    // return h <= y;
    // }
    //
    // Biome biome = worldObj.getBiomeGenForCoords(new BlockPos(x + chunkX * 16,
    // 0, z + chunkZ * 16));
    ...
    //
    // /**
    // * Digs out the current block, default implementation removes stone,
    // filler, and top block
    // * Sets the block to lava if y is less then 10, and air other wise.
    // * If setting to air, it also checks to see if we've broken the surface
    // and if so
    ...
    // */
    // protected void digBlock(Block[] data, int index, int x, int y, int z, int
    // chunkX, int chunkZ, boolean foundTop)
    // {
    // Biome biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ *
    // 16);
    // Block top = (isExceptionBiome(biome) ? Blocks.grass : biome.topBlock);
    // Block filler = (isExceptionBiome(biome) ? Blocks.dirt :
    // biome.fillerBlock);
    ...
    // }
    // else
    // if (ceiling && (data[index - 1] == Blocks.dirt || data[index-1] ==
    // Blocks.stone))
    // {
    // data[index - 1] = grass;
    // }
